...
def calib(df, k, bins=10, how='kumar'):
    '''
    train calibrator 
    '''
    
    prob_names = ["prob_" + str(i) for i in range(0,k)]
    probs = df[prob_names]
    if how=="kumar":
...
class KDataset:
    ...
    def __init__(self, df, k=2, cal_size=500, cal_bins=10, cal=True):
        self.k = k
        df = self.add_vars(df)
        self.df_cal = df.sample(n=cal_size, replace=False, random_state=0)
        self.df = df.drop(self.df_cal.index, axis=0).copy()
        if cal:
            hb_cal = calib(self.df_cal, self.k, how='hb')
            self.get_calibrated_probs(hb_cal, "_hb")
            kumar_cal = calib(self.df_cal, self.k, how='kumar')
            self.get_calibrated_probs(hb_cal, "_kumar")
            if k==2:
                self.add_calibrated_probs(self.logistic_calibrate(self.df, self.df_cal), "_log")
                self.add_calibrated_probs(self.beta_calibrate(self.df, self.df_cal), "_beta")
            else:
                self.dirichlet_calibrate()
    ...
    def add_vars(self, df):
        df['incorrect'] = df['actual'] != df['pred']
        if self.k==2:
            df['pred_error'] = 1-np.maximum(df['prob_0'], df['prob_1'])
        else:
            prob_names = ["prob_" + str(i) for i in range(0,self.k)]
            df['pred_error'] = df.apply(lambda x: 1-np.max(x[prob_names]), axis=1)
    ...
    def get_calibrated_probs(self, calibrator, label):
        ...
        if self.k==2:
            ...
            col_names = ["prob_"+str(i)+label for i in range(0,10)]
            tmp = pd.DataFrame(calibrated_probs, columns=col_names)
            self.df = pd.merge(self.df, tmp, left_index=True, right_index=True)
            self.df[cols[1]] = self.df.apply(lambda x: 1-np.max(x[col_names]), axis=1)
    ...
    def logistic_calibrate(self, test, cal):
    ...
    def beta_calibrate(self, test, cal):
    ...
    def dirichlet_calibrate(self):
        prob_names = ["prob_" + str(i) for i in range(0,self.k)]
        probs_cal = self.df_cal[prob_names]
        probs = self.df[prob_names]
        calibrator = FullDirichletCalibrator(reg_lambda=[1e-3], reg_mu=None)
        skf = StratifiedKFold(n_splits=3, shuffle=True, random_state=0)
        gscv = GridSearchCV(calibrator, param_grid={'reg_lambda':  [1e-3],
                                                    'reg_mu': [None]},
                            cv=skf, scoring='neg_log_loss')
        gscv.fit(np.array(probs_cal), np.array(self.df_cal['actual']))
        new_probs = gscv.predict_proba(np.array(probs))
        col_names = ["prob_"+str(i)+"_dirichlet" for i in range(0,10)]
        df_diri = pd.DataFrame(new_probs, columns=col_names)
        self.df = pd.merge(self.df, df_diri, left_index=True, right_index=True)
        self.df['pred_error_dirichlet'] = self.df.apply(lambda x: 1-np.max(x[col_names]), axis=1)
        self.df['err_diff_dirichlet'] = self.df['incorrect'].astype(int) - self.df['pred_error_dirichlet']
    ...
    def split_calibrate(self, var, splits, how='kumar'):
        ...
        col_names = ["prob_"+str(i)+"_split" for i in range(0,self.k)]
        prob_names = ["prob_" + str(i) for i in range(0,self.k)]
        ...
        for i in range(len(dfs)):
            if how=='kumar':
                ...
                probs_cal = df_cals[i][prob_names]
                probs = dfs[i][prob_names]
                calibrator = FullDirichletCalibrator(reg_lambda=[1e-3], reg_mu=None)
                skf = StratifiedKFold(n_splits=3, shuffle=True, random_state=0)
                gscv = GridSearchCV(calibrator, param_grid={'reg_lambda':  [1e-3],
                                                            'reg_mu': [None]},
                                    cv=skf, scoring='neg_log_loss')
                gscv.fit(np.array(probs_cal), np.array(df_cals[i]['actual']))
                new_probs = gscv.predict_proba(np.array(probs))
                df_diri = pd.DataFrame(new_probs, columns=col_names, index=dfs[i].index)
                updated_dfs.append(pd.merge(dfs[i], df_diri, left_index=True, right_index=True))
    ...
    def multi_split_calibrate(self, var, splits, how='dirichlet'):
        col_names = ["prob_"+str(i)+"_split" for i in range(0,self.k)]
        df_cals = [self.df_cal[self.df_cal[var]<splits[0]].copy()]
        dfs = [self.df[self.df[var]<splits[0]].copy()]
        for i in range(1, len(splits)):
        ...
        for i in range(len(dfs)):
            ...
            df_diri = pd.DataFrame(new_probs, columns=col_names, index=dfs[i].index)
            updated_dfs.append(pd.merge(dfs[i], df_diri, left_index=True, right_index=True))
            #dfs[i] = pd.merge(dfs[i], df_diri, left_index=True, right_index=True)
